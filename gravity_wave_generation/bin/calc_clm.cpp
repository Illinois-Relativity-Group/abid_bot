...
void calc_Clm(string const & filename, double m_adm, double cutoff_w=0.05, size_t padding=100000)
{
   ...
   for(size_t mode=0; mode<num_modes; mode++)
   {
      ...
      for(size_t i=0; i<padded_num_times; i++)
      {
         ...
         // Determine l and m for this mode
         int l = 2;
         int m = 0;
         int mode_index = mode;
         // Find l such that mode_index < 2*l+1, then m = mode_index - l
         while (mode_index >= 2 * l + 1) {
             mode_index -= 2 * l + 1;
             l++;
         }
         m = mode_index - l;

         double cutoff = cutoff_w * abs(m*l);

         if (abs(w) < cutoff && abs(m) > 0) {
             frequency_domain[i][0] *= -c / (cutoff * cutoff);
             frequency_domain[i][1] *= -c / (cutoff * cutoff);
         } else if (abs(m) > 0) {
             frequency_domain[i][0] *= -c / (w * w);
             frequency_domain[i][1] *= -c / (w * w);
         } else {
             // For m=0, keep original cutoff_w
             double cutoff0 = cutoff_w;
             if (abs(w) < cutoff0) {
              frequency_domain[i][0] *= -c / (cutoff0 * cutoff0);
              frequency_domain[i][1] *= -c / (cutoff0 * cutoff0);
             } else {
              frequency_domain[i][0] *= -c / (w * w);
              frequency_domain[i][1] *= -c / (w * w);
             }
...
{
   DataFile<double> output;
   output.resize(1+2*num_modes,num_times);
   // sample h+, hx and write to file
   for(size_t time=0; time<num_times; time++)
   {
      for(size_t mode=0; mode<num_modes; mode++)
      {
       output[time][0] = dt*time;
	     output[time][1+2*mode+0] = Clm[time*num_modes+mode].real();
		   output[time][1+2*mode+1] = Clm[time*num_modes+mode].imag();
      }
   }
   cout << "dt=" << dt << endl;
   cout << "num_times=" << num_times << endl;
...
int main (int argc, const char *argv[])
{
   ...
   write_simple_Clm(fol_name + "/Clm_with_time.dat");
   return 0;
}
