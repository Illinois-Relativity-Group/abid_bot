...
# AbidBot a
def setup_gw(gw):
    print(gw.start_time, gw.end_time)
    start_TOT = time.time()
    ...
    if gw.test_flag == 1: 
        ...
        """
        # the nice version with time
        gw.clm_f = np.loadtxt('/anvil/scratch/x-colson1/abid_bot_ALF2.7_1.38E16/bin/Clm_with_time.dat', dtype=float)
        num_times = gw.clm_f.shape[0]
        num_modes = (gw.clm_f.shape[1]-1)/2
        gw.clm = np.zeros((num_times,num_modes))
        for imode in range(0,num_modes):
          gw.clm[:,imode] = gw.clm_f[:,1+2*imode] + gw.clm_f[:,1+2*imode+1]*1j
        """
        #gw.clm_f = np.loadtxt('/anvil/scratch/x-colson1/abid_bot_ALF2.7_1.38E16/bin/gw_code/Clm_jamie', dtype=float)
