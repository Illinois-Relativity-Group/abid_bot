...
plt.plot(data0[offset:,0]/M, data0[offset:,1]*M, ls='-', lw=2, color=[0,0,0], label=r"$t/P_c = $"+str(int(np.ceil(10*first_t/period)/10)))
...
for i in timelist[1:2]:
	t_over_P = int(int(times[8])/(10*period))+1
...
for i in timelist[2:3]:
        ...
        plt.rc('legend', fontsize=8)
        plt.plot(data[i][offset:,0]/M,data[i][offset:,1]*M, ls='-', lw=2, color=colors, label= r'$t/P_c = $'+ str(t_over_P))
for i in timelist[3:4]:
	t_over_P = int(int(times[17])/(10*period))+1
...
for i in timelist[4:5]:
	...
	plt.rc('legend', fontsize=8)
	plt.plot(data[i][offset:,0]/M,data[i][offset:,1]*M, ls='-', lw=2, color=colors, label= r'$t/P_c = $'+ str(t_over_P))
for i in timelist[5:6]:
	t_over_P = int(int(times[26])/(10*period))+1
	colors = [1.,0.,0.]
	plt.rc('legend', fontsize=8)
	plt.plot(data[i][offset:,0]/M,data[i][offset:,1]*M, ls='-', lw=2, color=colors, label= r'$t/P_c = $'+ str(t_over_P))
'''
