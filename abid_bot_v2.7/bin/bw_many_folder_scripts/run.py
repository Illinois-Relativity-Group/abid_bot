...
if particles():
	stream_particles = PlotB(Bxdir, "particles", idx("particles"))
#add gridPoints-seeded Streamline Plot####
if gridPoints():
	stream_gridPoints = PlotB(Bxdir, "gridPoints", idx("gridPoints"))
#add bhplots##############################
if bh_formed():
...
for frame in range(firstFrame,lastFrame):
	...
	if density():
		LoadAttribute(extrasDir + volumeXML[state], vol)
	if bsq2r():
		LoadAttribute(bsq2rXML, bsq_atts)
	if particles():
