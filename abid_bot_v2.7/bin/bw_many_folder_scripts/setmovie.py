...
def getFolder(state):
	...
	frame_gap = 0
	for n in range(5):					# Skip the first 5 header lines
		f_overlap.readline()
...
def getTotalNumberOfFrames():
	...
	print("There are {} frames!".format(tot_number_of_frames))
...
run_mov_fixed_view(0, getTotalNumberOfFrames(), view_xml, vol2_xml)
