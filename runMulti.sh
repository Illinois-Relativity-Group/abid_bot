...
#foldernum=(49)    #the t/M-10000 folder, re-render after the sort fix
foldernum=()    #empty = use firstFolder..lastFolder; else list folders (1-indexed)
...
for dir in $(ls -d ${h5dir}"/"$h5prefix* ); do
	#folder selection: a non-empty foldernum overrides firstFolder..lastFolder
	if [ $all = true ]; then
		selected=1
