...
def cylinder_velocity(addOp):
        ...
        LoadAttribute("/scratch1/08796/jnd2/abid_bot_low_mass_m/bin/bw_many_folder_scripts/atts/Cylinder_atts.xml", CylAtts)
...
#############################################
        ...
        def LoadDatabases(self):
			...
			LoadandDefine(self.vxdir, 'vx')
                        ...
                        DefineVectorExpression("vVec","if(gt(logbsq2r, -0.5), vVec_temp, {0,0,0})")
...
#############################################
        ...
        def SetPlots(self):
                ...
                if self.particles():
                        ...
                        for i in range(self.numBfieldPlots):
                                self.stream_particles_dict["stream_particles_{}".format(i)] = PlotB(self.Bxdir, self.idx("particles{}".format(i)), ref=self.refPlot, ct='streamlines_{}'.format(i))
...
#############################################
        ...
        def SetAtts(self, frame, att_list):
                ...
                view, rho_vol, rho_pseudo, bsq2r, vector,spinvec, g00_att, seedfile, stream, vector2 = att_list
                ...
                if self.velocity():                     LoadAttribute(vector, self.vector1_atts)
...
#############################################
        ...
        def PlotFrame(self, frame, forceAddOp=0):
                ...
                if self.velocity():
                        ...
                        clip_vec(forceAddOp or frame==self.firstFrame)
                ...
                if self.velocity():
                        ...
                        #cylinder(self.CoM_x,self.CoM_y, 3, forceAddOp or frame==self.firstFrame, -100, height=100, spinfile=self.spinvtk[:-14]+"{:04d}".format(frame)+".vtk")
                        # cylinder_velocity(forceAddOp or frame==self.firstFrame)
                        clip_vec_inverse(forceAddOp or frame==self.firstFrame)
...
#############################################
        ...
        def PlotEvolve(self):
                ...
                for frame in range(self.firstFrame, self.lastFrame):
                        ...
                        movie_attributes.append('/scratch1/08796/jnd2/abid_bot_low_mass_m/bin/bw_many_folder_scripts/atts/Vec_fewer.xml')
