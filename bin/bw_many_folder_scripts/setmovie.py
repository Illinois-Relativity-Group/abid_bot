...
# Only get opacity attenuation and free form opacity. # Caveat: assumes opacity array at exactly 11th element, untrue in many cases
'''
...
def getVolVariables(volXML):
    ...
    freeform_opacity = None
    opacity_attenuation = None

    for field in data.findall(".//Field"):
        name = field.attrib.get("name", "")
        if name == "freeformOpacity":
            freeform_opacity = field.text
        elif name == "opacityAttenuation":
            opacity_attenuation = field.text

    if freeform_opacity is None:
        raise ValueError("freeformOpacity field not found in XML.")
    if opacity_attenuation is None:
        raise ValueError("opacityAttenuation field not found in XML.")

    # Convert opacity values
    mylist += [int(f) for f in freeform_opacity.split()]
    mylist.append(float(opacity_attenuation))  # index 256
    return mylist
...
def run_mov_change_attribute(first_frame, last_frame, view_initial, view_final, vol_initial, vol_final):
    ...
    for state in range(first_step,last_step,1):
        ...
        if fields: #need '- offset' as first file is at first time with particle data,
            ...
            print("hey")
            print(src_list + stream_src_list)
            print(particleseedpath)
            ...
            if twoColorsSeeds:
                src0 = gridseedpath + "{:0>7d}_0_0.txt".format(state)
                src1 = gridseedpath + "{:0>7d}_0_1.txt".format(state)
                src2 = attsdir + "/grid_Stream_3D_0.xml"
                src3 = attsdir + "/grid_Stream_3D_1.xml"
                dst0 = saveFolder + "grid_seeds_0_" + str(index).zfill(4) + ".txt"
                dst1 = saveFolder + "grid_seeds_1_" + str(index).zfill(4) + ".txt"
                dst2 = saveFolder + "grid_Stream_0_" + str(index).zfill(4) + ".xml"
                dst3 = saveFolder + "grid_Stream_1_" + str(index).zfill(4) + ".xml"
                shutil.copy2(src2,dst2); shutil.copy2(src3,dst3)
                if isfile(src0) and isfile(src1):
        ...
        f.close()
        ###########################
        bhtList = [ abs(x - myTime) for x in bhtimeList ]
        bhpos = bhtList.index(min(bhtList))
        print("bhpos")
        print(bhpos)
        ###copy bh1 cm and time data###
        bh1_cm = bh1_cm_List[bhpos]
