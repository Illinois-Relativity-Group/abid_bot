...
# bhcen1.txt is generated per-case by setup_bh.sh (center_lister.py) from your
# own horizon data, so it is not in the repo. setup.sh only runs setup_bh.sh
# when h5data/horizon/ exists -- say so plainly rather than raising a bare
# FileNotFoundError from three layers down.
if not os.path.isfile(bh1_cm_file):
    sys.exit("setmovie: %s is missing.\n"
             "  It is generated by setup_bh.sh, which setup.sh runs only when\n"
             "  h5data/horizon/ exists. Put your horizon .gp files there\n"
             "  (horizon/all_horizon/) and re-run setup.sh." % bh1_cm_file)
