...
if PlotZoom or PlotFlyOver or PlotFlyAround:
	idx = int(sys.argv[35])
	num_frames = int(sys.argv[36])
	view_initial = sys.argv[37]
	vol_initial = sys.argv[38]
	miscatts = [idx, num_frames, view_initial, vol_initial]
	if PlotZoom:
		view_final = sys.argv[39]
		vol_final = sys.argv[40]
...
print(sys.argv)
PlotOpts = [ PlotDensAsVol, PlotDensAsIso, PlotDensLinear, PlotVel, PlotSpinVec, PlotBsq2rAsVol, PlotBsq2rAsIso, PlotVelCustom, Plotg00, refPlot,\
			 cutPlot, bgcolor ]
ArgList  = [ h5dir, extrasDir, saveFolder, rank, total_ranks, numBfieldPlots, vector1XML, vector2XML, SpinVecXML,\
			 bsq2rXML, bsq2r_pseudoXML, bsq2r_isoXML, max_density, rho_pseudoXML, rho_isoXML, g00_pseudoXML, g00_isoXML, VelCustomFile]
