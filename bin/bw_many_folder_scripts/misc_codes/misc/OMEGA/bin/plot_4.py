...
dt=float(argv[2])
n_frac=float(argv[3])
offset=3 #number of points to ignore near origin
...
plt.plot(data0[offset:,0]/M, data0[offset:,1]*M, lw=2, color=[0.6,0.8,1], label="t/P="+str(first_t))
plt.scatter(data0[offset:,0]/M, data0[offset:,1]*M)
...
step=int(np.ceil(n_thread*1.0/10))
if step==0:
	step=1
for i in range(step,n_thread,step):
	blues=[float(3*(n_thread-i))/(5*n_thread),float(4*(n_thread-i))/(5*n_thread),float((n_thread-i))/n_thread]
	t_over_P=int(int(times[i])/period)+1
	plt.rc('legend', fontsize=11)
	plt.plot(data[i][offset:,0]/M,data[i][offset:,1]*M, '-', lw=2, color=blues, label= '       '+ str(t_over_P))
...
plt.legend()
#plt.show()
plt.savefig("png_ave/"+str(n_frac)+"Times_mode4_single.png", dpi=300)
