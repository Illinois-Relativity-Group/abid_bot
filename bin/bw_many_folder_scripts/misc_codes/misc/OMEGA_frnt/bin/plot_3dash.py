...
plt.plot(data0[offset:,0]/M, data0[offset:,1]*M, ls=(0, (5,7)), lw=4, color=[1,0,0], label="t/P=0")
...
for i in [0,2,4]:
	...
	plt.rc('legend', fontsize=11)
	plt.plot(data[i][offset:,0]/M,data[i][offset:,1]*M, ls=(0,(5,3)), lw=2, color=colors, label= '       '+ str(t_over_P))

for i in [6,8,10]:
	t_over_P = int(int(times[i])/period)+1
	colors = [95/255.,158/255.,160/255.]
	plt.rc('legend', fontsize=11)
	plt.plot(data[i][offset:,0]/M,data[i][offset:,1]*M, ls=(0,(7,4,2,4)), lw=2, color=colors, label= '       '+ str(t_over_P))

for i in [12,14,16]:
	t_over_P = int(int(times[i])/period)+1
	colors = [65/255.,105/255.,225/255.]
	plt.rc('legend', fontsize=11)
	plt.plot(data[i][offset:,0]/M,data[i][offset:,1]*M, ls=(0,(2,1.5)), lw=2, color=colors, label= '       '+ str(t_over_P))
'''
for i in [18,20]:
        t_over_P = int(int(times[i])/period)+1
	colors = [0.0,0.0,0.0]
