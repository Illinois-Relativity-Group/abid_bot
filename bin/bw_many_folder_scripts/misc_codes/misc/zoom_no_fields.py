...
#turn off B-field without changing original xml files.
def particles():
	return 0 #len(particlesTXT) > 0

def gridPoints():
	return 0 #len(gridPointsTXT) > 0		

def fields():
	return 0 #particles() or gridPoints()
...
def zoom_fixed_time(zoomsteps, view_initial, view_final, frame_start, frame_end):
	print "Zooming"
	cpts=(view_initial, view_final)
	x=[0,1]
	for my_i in range(frame_start, frame_end):
		t=float(my_i)/float(zoomsteps-1)
		c=EvalCubicSpline(t,x,cpts)
		SetView3D(c)
		SaveWindow()

zoom_fixed_time(tot_frames, c0, c1, frame_start, frame_end)
