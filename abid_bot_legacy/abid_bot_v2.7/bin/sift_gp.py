...
time_list = [ int(f[3:-7]) for f in listdir(all_dir) if isfile(join(all_dir,f)) and f.find("h.t")  != -1  and f.find(".ah" + ahtype + ".gp") != -1]

max_time = float(max(time_list))
min_time = float(min(time_list))
first_time = int(round(min_time/iter,0)*iter)
last_time = int(round(max_time/iter,0)*iter)
