...
for i in files_temp:
	#you almost certainly will want to change this condition based on when your stars merge/bh forms
	if(i[0]!='w') or (int(i[2:7])<=14300) or (int(i[2:7]) >= 18400):
		continue
	w_files.append(i)
w_files.sort()
...
def calc_N():
	...
	#ignoring first two points since usually nan. can add or remove more as needed
	line=fil.readline()
	line=fil.readline()
	line=fil.readline()
	w=float(line.strip().split()[1])
	
	times=[int((float(i[2:-4])*M-ini_time)/(10*dt)) for i in w_files]
	print ('times',times)
