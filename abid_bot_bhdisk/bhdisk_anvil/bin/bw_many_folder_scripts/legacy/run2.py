...
if PlotDensAsIso:
		rho_pseudoXML = sys.argv[15]
		rho_isoXML = sys.argv[16]
if Plotg00:
		g00_pseudoXML = sys.argv[17]
		g00_isoXML = sys.argv[18]
...
##################################################
#Choose what to plot
def density_vol(): return PlotDensAsVol
#
def density_iso(): return PlotDensAsIso
#
def density_linear(): return PlotDensLinear
#
def bsq2r(): return PlotBsq2r and not density_vol() #Can't have 2 volume plots
#
...
print("density: {}".format(density_vol() or density_iso()))
...
###
if g00():
	dbs += [psidir, shiftxdir, shiftydir, shiftzdir, gxxdir, gyydir, gzzdir, gxydir, gxzdir, gyzdir, lapm1dir]
	plot_idx += ["g00"]
...
if density_iso():
	if density_linear():
		...
		rho_atts = PlotPseudo(rho_bdir, "logrho", idx("density"))
#add bsq2r Plot ##################
if bsq2r():
...
for frame in range(firstFrame,lastFrame):
	...
	if density_vol(): print(extrasDir + volumeXML[state])
	time.strftime("%Y-%m-%d %H:%M:%S")

	print("Loading Attibutes")
	LoadAttribute(extrasDir + viewXML[state], myView)

	if density_vol():		LoadAttribute(extrasDir + volumeXML[state], vol)
	if density_iso():		LoadAttribute(rho_pseudoXML, rho_atts)
