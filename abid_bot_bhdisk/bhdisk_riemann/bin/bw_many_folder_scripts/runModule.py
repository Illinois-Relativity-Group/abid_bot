import sys
sys.path.insert(0, "/data/codyolson/.venv/lib/python3.12/site-packages/numpy")
from visit import *
...
def setAnnotations(lightlist=[]):#sets background, sets up text
        ...
        Ann.legendInfoFlag = 1 # 0/1 to turn off/on color_bar
        Ann.databaseInfoFlag = 0
        Ann.userInfoFlag = 0
        Ann.axes3D.visible = 0
        Ann.axes3D.triadFlag = 0
        Ann.axes3D.bboxFlag = 0 # or 0
...
def getLists(extrasDir, numBfieldPlots=1):
        ...
        fileNames1 = ["volume_", "grid_seeds_0_", "grid_seeds_1_", "view_", "time_", "bh1_", "bh2_", "bh3_", "trace1_", "trace2_", "spin_", "grid_Stream_0", "grid_Stream_1"]
...
def PlotBox():
        ...
        AddPlot("Pseudocolor", "test")  # Replace "default" with the correct variable if needed

        # Set pseudocolor attributes to make the box white
        p = PseudocolorAttributes()
        p.colorTableName = "white"     # Use a solid color instead of a gradient
        #    p.singleColor = (255, 255, 255, 255)  # Pure white RGBA
        #    p.colorType = p.ColorBySingleColor     # Force single color mode
        #    p.opacity = 1.0
        p.legendFlag = 1
        p.pointType = p.Point
        p.pointSize = 0.5
...
def PlotTrace(database, idx, indx):
        ...
        pointAtt.maxFlag = 1
        pointAtt.max = 0
        pointAtt.legendFlag = 1 #color_bar
...
class VisitPlot:
        ...
        def __init__(self, plot_opts, arg_list):
                ...
                (self.volumeXML, self.gridPointsTXT_0, self.gridPointsTXT_1, self.viewXML, self.timeTXT, self.bh13D, self.bh23D, self.bh33D, self.trace3D, self.trace23D, self.spinvtk, self.gridStream_0, self.gridStream_1, self.particlesDict, self.StreamDict, self.stateList) = getLists(self.extrasDir, self.numBfieldPlots)
        ...
        def __repr__(self):
                plots = ['PlotDensAsVol', 'PlotDensAsIso', 'PlotDensLinear', 'PlotVel', 'PlotSpinVec', 'PlotBsq2rAsVol', 'PlotBsq2rAsIso',  'Plotg00', 'refPlot', 'cutPlot']
                args = ['h5dir', 'extrasDir', 'saveFolder', 'rank', 'total ranks', 'numBfieldPlots', 'vectorXML', 'vector2XML','spinvecXML', 'bsq2rXML', 'bsq2r_pseudoXML', 'bsq2r_isoXML', 'max_density', 'rho_pseudoXML', 'rho_isoXML', 'g00_pseudoXML', 'g00_isoXML']
...
#############################################
        ...
        def LoadDatabases(self):
                ...
                if self.density_vol() or self.density_iso():
                        ...
                        print('yinuan temp, max_density' + self.max_density)
                        dbs += [self.rho_bdir]
                        plot_idx += ["density"]
...
#############################################
        ...
        def SetAtts(self, frame, att_list):
                ...
                view, rho_vol, rho_pseudo, bsq2r, bsq2r_pseudo, bsq2r_iso, vector1, vector2, spinvec, g00_att, seedfile, stream = att_list
...
#                            pass
                ...
                if self.spinvec():
                        ...
                        print("spinvec atts:")
                        print(self.spinvec_atts)
                        SetPlotOptions(self.spinvec_atts)
                        #spherespinvec doesn't center correctly
                        boxspinvec(self.CoM_x,self.CoM_y,self.CoM_z, forceAddOp or frame==self.firstFrame) #original line, puts spinvec at the CoM.
                        #boxspinvec(-3.31781605e-18, -3.31781605e-18,  7.96912807e-02, forceAddOp or frame==self.firstFrame) #use this for first frame
                        #print('custom start point of spinvec at: -3.31781605e-18, -3.31781605e-18,  7.96912807e-02') #use this for first frame
                        print("spinvec set")
                        #if self.cutPlot: box(self.CoM_y, forceAddOp or frame==self.firstFrame)
                if self.g00():
...
#############################################
        ...
        def PlotEvolve(self):
                ...
                for frame in range(self.firstFrame, self.lastFrame):
                        ...
                        movie_attributes = [viewAtt, volAtt, self.rho_pseudoXML, self.bsq2rXML, self.bsq2r_pseudoXML, self.bsq2r_isoXML, self.vector1XML, self.vector2XML, self.spinvecXML, self.g00_pseudoXML,'','']
...
#############################################
        ...
        def PlotZoom(self, zoomopts):
                ...
                for my_i in range(frame_i, frame_f):
                        ...
                        movie_attributes = [c, v, self.rho_pseudoXML, self.bsq2rXML, self.bsq2r_pseudoXML, self.bsq2r_isoXML, self.vector1XML, self.vector2XML, self.spinvecXML, self.g00_pseudoXML, '','']
                        self.SetAtts(frame, movie_attributes)
...
#############################################
        ...
        def PlotFlyOver(self, zoomopts):
                ...
                for i in range(frame_i, frame_f):
                        ...
                        movie_attributes = [newView, myVol, self.rho_pseudoXML, self.bsq2rXML, self.bsq2r_pseudoXML, self.bsq2r_isoXML, self.vector1XML, self.vector2XML, self.spinvecXML, self.g00_pseudoXML, '','']
                        self.SetAtts(frame, movie_attributes)
                        self.PlotFrame(frame, forceAddOp=1)
