...
void calc_Clm(string const & filename, double r_phys, double m_adm, double cutoff_w=0.020679, size_t padding=1)
{
   // load Psi4
   ...
   for(size_t k=0; k<nz; k++)
   {
      ...
      for(size_t j=0; j<ny; j++)
      {
         ...
         for(size_t i=0; i<nx; i++)
         {
            ...
            Ylm[kji*num_modes+0]=calc_Ylm(2,2,theta,phi);
            Ylm[kji*num_modes+4]=calc_Ylm(2,-2,theta,phi);
         }
      }
   }
      ...
      for(size_t k=0; k<nz; k++)
         for(size_t j=0; j<ny; j++)
            for(size_t i=0; i<nx; i++)
            {
               ...
               hphc+=(Clm[rt*num_modes+0]*(1.0-drt)+Clm[(rt+1)*num_modes+0]*drt)*Ylm[kji*num_modes+0];
               hphc+=(Clm[rt*num_modes+4]*(1.0-drt)+Clm[(rt+1)*num_modes+4]*drt)*Ylm[kji*num_modes+4];
...
void write_1D(string const & filename, double theta, double phi)
{
   ...
   for(size_t time=0; time<num_times; time++)
   {
      ...
      int l=2, m=2;
      complex<double> hphc;
      /*for(size_t mode=0; mode<5+7+9+11; mode++) //num_modes
      {
      ...
      hphc+=Clm[time*num_modes+0]*calc_Ylm(2,2,theta,phi);
      hphc+=Clm[time*num_modes+4]*calc_Ylm(2,-2,theta,phi);
...
int main (int argc, const char *argv[])
{
   ...
   //write_3D("3D",n,n,n/2,-max,max,-max,max,-max,0,1);
	...
	calc_Clm("Psi4_rad.mon.sort.1", 120.0, 7.895769590229179, 0.01); //TODO (filename, radius, ADM mass, cutoff_w)
