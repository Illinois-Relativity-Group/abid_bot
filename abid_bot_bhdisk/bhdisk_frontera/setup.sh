...
spinf1=$root/h5data/bhns_BHspin.mon
if [[ -f "$spinf1" && "$spin_dim" != "false" && -n "$spin_dim" ]];then
	echo "dimension spin: $spin_dim"
	python3 $bin/setup_spinvtk_dimension.py $root $M $setN
fi

if [[ -f "$spinf1" && "$spin_dimless" != "false" && -n "$spin_dimless" ]];then
	echo "dimensionless spin: $spin_dimless"
	python3 $bin/setup_spinvtk_dimensionless.py $root $M $setN
fi
